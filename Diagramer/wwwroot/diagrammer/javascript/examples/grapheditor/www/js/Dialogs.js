...
var RegisterDialog = function(editorUi)
{
	var div = document.createElement('div');
	div.setAttribute('align', 'center');
	var h3 = document.createElement('h3');
	mxUtils.write(h3, 'Register account');
	div.appendChild(h3);
	h3.style.marginBottom = "25px";

	var h4 = document.createElement('h3');
	mxUtils.write(h4, 'Email:');
	h4.style.fontSize = "12px";
	div.appendChild(h4);

	var inputName = document.createElement('input');
	div.appendChild(inputName);
	inputName.style.marginLeft = "12px";

	var h5 = document.createElement('h3');
	mxUtils.write(h5, 'Password:');
	...
	inputPassword.style.marginBottom = "50px";
	inputPassword.style.marginLeft = "12px";

	var btCreateAccount = document.createElement('input');
	btCreateAccount.type = "button";
	btCreateAccount.value = "Создать аккаунт";
	btCreateAccount.style.marginLeft = "12px";
	btCreateAccount.style.height = "28px";
	btCreateAccount.setAttribute("onclick", "AcceptCreateAccount()");
	div.appendChild(btCreateAccount);
...
var LoginDialog = function(editorUi)
{
	var div = document.createElement('div');
	div.setAttribute('align', 'center');
	var h3 = document.createElement('h3');
	mxUtils.write(h3, 'Login account');
	div.appendChild(h3);
	h3.style.marginBottom = "25px";
	...
	var btCreateAccount = document.createElement('input');
	btCreateAccount.type = "button";
	btCreateAccount.value = "Войти в аккаунт";
	btCreateAccount.style.marginLeft = "12px";
	btCreateAccount.style.height = "28px";
	btCreateAccount.setAttribute("onclick", "AcceptCreateAccount()");
	div.appendChild(btCreateAccount);
...
var inputText;
var ExtractTextDialog = function(editorUi)
{
	var div = document.createElement('div');
	div.setAttribute('align', 'center');

	var h3 = document.createElement('h3');
	mxUtils.write(h3, 'Извлеченные текста:');
	div.appendChild(h3);

	inputText = document.createElement('textarea');
	inputText.style.width = "350px";
	inputText.style.height = "350px";

	inputText.value = needExtractText;
	div.appendChild(inputText);

	var closeBtn = mxUtils.button('C#', function()
	{
		AcceptCsharp();
	});
	div.appendChild(closeBtn);
...
var AcceptCreateAccount = function()
{

};
var AcceptCsharp = function()
{
	var labelsTLegnth = labelsInExT.length;
	var labelsTLengthStart = 0;
	var classIsOpen = 0;


	inputText.value = "using System;" + "\n" + "\n" + "namespace Diagram" + "\n" + "{";

	while(labelsTLengthStart < labelsTLegnth)
	{
		var nameParam = labelsInExT[labelsTLengthStart].toString().split(' ');

		if( labelsInExT[labelsTLengthStart].includes("+") ||
			labelsInExT[labelsTLengthStart].includes("-"))
		{
			//Атрибуты
			if (labelsInExT[labelsTLengthStart].includes("+") &&
				labelsInExT[labelsTLengthStart].includes(":")&&
				labelsInExT[labelsTLengthStart].includes("(") === false) {
				inputText.value += "\n" + "      " + "public " + nameParam[2] + " " + nameParam[1].replace(":", "") + ";";
			}
			if (labelsInExT[labelsTLengthStart].includes("-") &&
				labelsInExT[labelsTLengthStart].includes(":") &&
			    labelsInExT[labelsTLengthStart].includes("(") === false){
				inputText.value += "\n" + "      " + "private " + nameParam[2] + " " + nameParam[1].replace(":", "")+ ";";
			}
			//Методы
			if (labelsInExT[labelsTLengthStart].includes("+") &&
				labelsInExT[labelsTLengthStart].includes("(") &&
		...
		else
		{
			...
			inputText.value += "\n" + "   ";
			inputText.value += labelsInExT[labelsTLengthStart].replace(labelsInExT[labelsTLengthStart],
				"class " + labelsInExT[labelsTLengthStart] + "\n" + "   " + "{");
			classIsOpen = 1;
		}

		if(labelsTLengthStart + 1 === labelsTLegnth){
			inputText.value += "\n" +  "   " + "}" ;
			classIsOpen = 0;
		}
		labelsTLengthStart++;
