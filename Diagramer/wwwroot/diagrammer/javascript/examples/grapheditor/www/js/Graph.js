 ...
 * Defines graph class.
 */
Graph = function(container, model, renderHint, stylesheet, themes, standalone, socket)
{
	mxGraph.call(this, container, model, renderHint, stylesheet,socket);
	...
	this.updateSocket = function(socket){
		...
		//model.createSocket(this.socket);
	}
	...
	for (var key in this.model.cells)
	{
		...
		if (this.model.isVertex(cell) || this.model.isEdge(cell))
		{
			if (this.isHtmlLabel(cell))
			{
				...
				labelInExT = mxUtils.extractTextWithWhitespace([tmp]);
			}
			else
			{
				labelInExT = this.getLabel(cell);
			}

			labelInExT = mxUtils.trim(labelInExT.replace(/[\x00-\x1F\x7F-\x9F]|\s+/g, ' '));
			
			if (labelInExT.length > 0)
			{
				labelsInExT.push(labelInExT);
			}
		}
	}
	
	return labelsInExT.join("\n");
};
