...
Actions.prototype.init = function () {
    ...
    this.addAction('save', function () {
        ...
        fetch(window.NET_URL + "api/" + window.DIAGRAM_ID, {
            method: "POST",
    ...
    this.put('extractText', new Action(' Извлечь текста', function () {
        var graph = ui.editor.graph;
        needExtractText = graph.getIndexableText(
            (graph.isSelectionEmpty()) ? null :
                graph.getSelectionCells());

        ui.showDialog(new ExtractTextDialog(ui).container, 450, 450, true, true);

    }));

    this.put('register', new Action(' Register', function () {
        //действия при клике на рег
        ui.showDialog(new RegisterDialog(ui).container, 320, 280, true, true, function () {
            showingAbout = false;
        });
    }));
    this.put('login', new Action(' Login', function () {
        ui.showDialog(new LoginDialog(ui).container, 320, 280, true, true, function () {
            showingAbout = false;
        });
    }));
